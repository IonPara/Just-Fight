  ...
  window.onload = () => {
    document
      ...
      .addEventListener("submit", function (event) {
        ...
        emailjs.sendForm("service_ymbbgkm", "contact_form", this).then(
          function () {
...
function Comments(comment, name) {
  (this.comment = comment), (this.name = name);
}
...
let hasPageRunBefore = () => {
  ...
  if (localStorage.getItem("hasPageRun")) {
    ...
    localStorage.setItem(
      "martialArtArticles",
      JSON.stringify(martialArtArticles)
    );
...
const showArticle = () => {
  ...
  const martialArtArticles = JSON.parse(
    localStorage.getItem("martialArtArticles")
  );
